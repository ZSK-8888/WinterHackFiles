...
def determine_frontier_path(state: SimulationState) -> None:
    ...
    if state.frontier_goal == state.goal["cell"]:
        ...
        # Check if the goal cell is already in the detected frontiers.
        if state.goal['cell'] in frontiers:
            state.frontier_goal = state.goal['cell']
        ...
        W_PROJECTION = 2.0   # Weight to encourage moving "forward"
        W_ALIGNMENT = 1.0    # Weight to encourage "alignment"
        W_ROBOT_DIST = 0.5   # Weight to prefer frontiers "far" from the robot
        W_GOAL_DIST = 1.0    # Weight to prefer frontiers "near" the final goal
        def get_weighted_score(cell: Cell) -> float:
            """
            Calculates the weighted score for a frontier cell.
            Higher score is better.
            """
            
            # Factor 1: Heading projection (higher is better)
            score_projection = W_PROJECTION * heading_projection(cell)
            # Factor 2: Heading alignment (higher is better)
            score_alignment = W_ALIGNMENT * heading_alignment(cell)
            # Factor 3: Distance from robot (higher is better, per effective weights)
            score_robot_dist = W_ROBOT_DIST * distances[cell]
            # Factor 4: Distance to final goal (lower is better -> negative sign)
            dist_to_goal = math.hypot(
                cell_center(cell, cell_size)[0] - goal_center[0],
                cell_center(cell, cell_size)[1] - goal_center[1],
            )
            score_goal_dist = -W_GOAL_DIST * dist_to_goal
            return score_projection + score_alignment + score_robot_dist + score_goal_dist

        if not pool:
            # if empty force a plan to the final goal cell as a last resort.
            best_frontier_cell = goal_cell
        else:
            best_frontier_cell = max(
                pool,
                key=lambda cell: (
                    get_weighted_score(cell)
                ),
            )
